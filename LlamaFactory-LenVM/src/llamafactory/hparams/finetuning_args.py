...
class FinetuningArguments(
    ...
    stage: Literal["pt", "sft", "rm", "ppo", "dpo", "kto", "lenvm"] = field(
        default="sft",
    ...
    # Length Value Model (LenVM) specific options
    lenvm_loss_type: Literal["huber", "bce"] = field(
        default="huber",
        metadata={"help": "Loss type for LenVM training. 'huber' uses Huber/MSE on transformed predictions; "
                          "'bce' uses binary cross-entropy with logits against target 1-gamma^l."},
    )
    lenvm_huber_loss_delta: float = field(
        default=1.0,
        metadata={"help": "Delta parameter for Huber (SmoothL1) loss in LenVM training."},
    )
    lenvm_gamma: float = field(
        default=0.999,
        metadata={"help": "Gamma parameter for GAE in LenVM training."},
    )
    lenvm_lam: float = field(
        default=1.0,
        metadata={"help": "Lambda parameter for GAE in LenVM training."},
    )
    lenvm_target_type: Literal["discounted_return", "length", "log_length"] = field(
        default="discounted_return",
        metadata={"help": "Regression target type for LenVM training."},
    )
    lenvm_length_normalizer: float = field(
        default=1.0,
        metadata={"help": "Normalization denominator used only when `lenvm_target_type=length`."},
    )
    lenvm_length_activation: Literal["softplus", "sigmoid"] = field(
        default="softplus",
        metadata={"help": "Activation used only when `lenvm_target_type=length`."},
    )
    lenvm_agg_method: Literal["token-mean", "seq-mean-token-sum", "seq-mean-token-mean", "seq-mean-token-mean-max"] = field(
        default="token-mean",
        metadata={"help": "Method to aggregate the loss in LenVM training."},
    )
    lenvm_alpha: float = field(
        default=0.0,
        metadata={"help": "Alpha parameter for LenVM training (from VAPO paper)."},
    )
    lenvm_relative_loss: bool = field(
        default=False,
        metadata={"help": "Whether to use the relative loss in LenVM training."},
    )
    lenvm_pos_metrics: list[float] | str = field(
        default_factory=lambda: [0.0, 0.25, 0.5, 0.75],
        metadata={"help": "Positions (fractions) to log per-position LenVM losses, e.g. 0,0.25,0.5,0.75."},
    )
    ...
    def __post_init__(self):
        ...
        self.lenvm_pos_metrics: list[float] = [float(x) for x in split_arg(self.lenvm_pos_metrics)]
        ...
        assert 0.0 < self.lenvm_gamma <= 1.0, "gamma must be (0.0, 1.0]"
        assert 0.0 <= self.lenvm_lam <= 1.0, "lam must be [0.0, 1.0]"
        assert self.lenvm_target_type in {"discounted_return", "length", "log_length"}, (
            "lenvm_target_type must be one of {'discounted_return', 'length', 'log_length'}."
        )
        assert self.lenvm_length_normalizer > 0.0, "lenvm_length_normalizer must be > 0."
        assert self.lenvm_length_activation in {"softplus", "sigmoid"}, (
            "lenvm_length_activation must be one of {'softplus', 'sigmoid'}."
        )
        assert self.lenvm_loss_type in {"huber", "bce"}, "lenvm_loss_type must be one of {'huber', 'bce'}."
        if self.lenvm_loss_type == "bce":
            assert self.lenvm_target_type == "discounted_return", (
                "lenvm_loss_type='bce' only works with lenvm_target_type='discounted_return'."
            )
        assert 0.0 <= self.lenvm_huber_loss_delta, "delta must be [0.0, inf)"
        for pos in self.lenvm_pos_metrics:
            assert 0.0 <= pos <= 1.0, "lenvm_pos_metrics must be within [0, 1]."
