...
from .model_utils.valuehead import MultiModalValueHeadModel, load_valuehead_params
...
def load_model(
    ...
    if add_valuehead:
        ...
        if type(config) in AutoModelForImageTextToText._model_mapping.keys():
            logger.info_rank0("Using multimodal value-head wrapper: MultiModalValueHeadModel.")
            model = MultiModalValueHeadModel(model, **vhead_kwargs)
        else:
            logger.info_rank0("Using text value-head wrapper: AutoModelForCausalLMWithValueHead.")
