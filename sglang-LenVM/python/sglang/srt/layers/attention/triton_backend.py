...
class TritonAttnBackend(AttentionBackend):
    def __init__(
        ...
        # Debugging: validate custom mask bookkeeping to catch overflow/mismatch early.
        # This is intentionally off by default to avoid any overhead on hot paths.
        self.debug_custom_mask = get_bool_env_var("SGLANG_DEBUG_CUSTOM_MASK", "false")
    ...
    def init_forward_metadata(self, forward_batch: ForwardBatch):
        ...
        if forward_batch.forward_mode.is_decode_or_idle():
            ...
            # IMPORTANT: compute in int64 to avoid int32 overflow when prompts are long and bs is large.
            # Overflow here corrupts mask_indptr and can lead to invalid attention reads -> NaN/Inf.
            seq_lens_i64 = forward_batch.seq_lens.to(torch.int64)
            seq_mask_len = (seq_lens_i64 + int(self.num_draft_tokens)) * int(
                self.num_draft_tokens
            )
            mask_indptr = self.mask_indptr
            mask_indptr[1 : bs + 1] = torch.cumsum(seq_mask_len[:bs], dim=0)
            mask_indptr = mask_indptr[: bs + 1]
            if self.debug_custom_mask:
                expected = int(spec_info.custom_mask.numel())
                got = int(mask_indptr[-1].item())
                if got != expected:
                    raise RuntimeError(
                        f"custom_mask/mask_indptr mismatch (target_verify): {got=} {expected=}"
                    )
            ...
            # Support spec_info.custom_mask for tree-style attention in EXTEND mode (non-speculative usage).
            if spec_info is not None and getattr(spec_info, "custom_mask", None) is not None:
                custom_mask = spec_info.custom_mask
                # IMPORTANT: compute in int64 to avoid overflow for large batches / long prompts.
                # Fast path (default): derive per-request mask length from forward_batch lens:
                #   Q_len = extend_seq_len
                #   K_len = extend_prefix_len + extend_seq_len
                #   mask_len = Q_len * K_len
                extend_i64 = forward_batch.extend_seq_lens.to(torch.int64)
                prefix_i64 = forward_batch.extend_prefix_lens.to(torch.int64)
                seq_mask_len = extend_i64 * (prefix_i64 + extend_i64)

                # Debug-only: verify that forward_batch lens align with tree_value spec metadata.
                if self.debug_custom_mask and (
                    hasattr(spec_info, "tree_value_prefix_lens")
                    and hasattr(spec_info, "tree_value_candidate_lens")
                    and hasattr(spec_info, "tree_value_cached_prefix_lens")
                ):
                    Ls = getattr(spec_info, "tree_value_prefix_lens")
                    Ns = getattr(spec_info, "tree_value_candidate_lens")
                    Ps = getattr(spec_info, "tree_value_cached_prefix_lens")
                    # Convert to int64 tensor on device for checking.
                    L = torch.tensor([int(x) for x in Ls[:bs]], dtype=torch.int64, device=self.device)
                    N = torch.tensor([int(x) for x in Ns[:bs]], dtype=torch.int64, device=self.device)
                    P = torch.tensor([int(x) for x in Ps[:bs]], dtype=torch.int64, device=self.device)
                    q = (L - P) + N
                    k = L + N
                    extend_chk = extend_i64[:bs]
                    prefix_chk = prefix_i64[:bs]
                    if not torch.equal(q, extend_chk):
                        diff = (q != extend_chk).nonzero(as_tuple=False)
                        j = int(diff[0].item()) if diff.numel() else -1
                        raise RuntimeError(
                            f"tree_value Q_len mismatch vs forward_batch.extend_seq_lens: "
                            f"idx={j} q={int(q[j]) if j>=0 else 'NA'} "
                            f"extend={int(extend_chk[j]) if j>=0 else 'NA'} bs={bs}"
                        )
                    if not torch.equal(k, prefix_chk + extend_chk):
                        diff = (k != (prefix_chk + extend_chk)).nonzero(as_tuple=False)
                        j = int(diff[0].item()) if diff.numel() else -1
                        raise RuntimeError(
                            f"tree_value K_len mismatch vs forward_batch prefix+extend: "
                            f"idx={j} k={int(k[j]) if j>=0 else 'NA'} "
                            f"prefix+extend={int((prefix_chk+extend_chk)[j]) if j>=0 else 'NA'} bs={bs}"
                        )
                mask_indptr = self.mask_indptr
                mask_indptr[1 : bs + 1] = torch.cumsum(seq_mask_len[:bs], dim=0)
                mask_indptr = mask_indptr[: bs + 1]
                if self.debug_custom_mask:
                    expected = int(custom_mask.numel())
                    got = int(mask_indptr[-1].item())
                    if got != expected:
                        raise RuntimeError(
                            f"custom_mask/mask_indptr mismatch (extend): {got=} {expected=}"
                        )
            else:
                custom_mask = None
                mask_indptr = None
    ...
    def init_forward_metadata_capture_cuda_graph(
        ...
        if forward_mode.is_decode_or_idle():
            ...
            # IMPORTANT: compute in int64 to avoid int32 overflow.
            ...
            if self.debug_custom_mask:
                ...
                if got != expected:
                    raise RuntimeError(
                        f"custom_mask/mask_indptr mismatch (cuda_graph_capture target_verify): {got=} {expected=}"
                    )
            max_extend_len = self.num_draft_tokens
            num_kv_splits = None
            attn_logits = None
    ...
    def init_forward_metadata_replay_cuda_graph(
        ...
        if forward_mode.is_decode_or_idle():
            ...
            if self.debug_custom_mask:
                ...
                if got != expected:
                    raise RuntimeError(
                        f"custom_mask/mask_indptr mismatch (cuda_graph_replay target_verify): {got=} {expected=}"
                    )
        elif forward_mode.is_draft_extend(include_v2=True):
