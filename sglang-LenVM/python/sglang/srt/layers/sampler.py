...
class Sampler(nn.Module):
    ...
    def _apply_token_temp_scale(
        self, logits: torch.Tensor, reqs
    ) -> None:
        """In-place divide logits of boosted tokens by their divisor.

        After logits.div_(temperatures), logits[i,j] = raw_logit / T.
        Dividing by divisor gives raw_logit / (T / divisor) = raw_logit * divisor / T,
        equivalent to temperature T/divisor for those tokens.
        """
        from sglang.srt.lvm.lvm_guided_sampling import _extract_token_temperature_scale
        ...
        for i, req in enumerate(req_list):
            scale_map = _extract_token_temperature_scale(req)
            if not scale_map:
                continue

            cached = getattr(req, "_lvm_token_temp_tensors", None)
            if cached is None:
                ids_list = list(scale_map.keys())
                div_list = [scale_map[k] for k in ids_list]
                ids_t = torch.tensor(ids_list, dtype=torch.long, device=device)
                div_t = torch.tensor(div_list, dtype=logits.dtype, device=device)
                try:
                    setattr(req, "_lvm_token_temp_tensors", (ids_t, div_t))
                except Exception:
                    pass
                cached = (ids_t, div_t)
            ...
            if ids_t.device != device:
                ...
                div_t = div_t.to(device)

            valid = (ids_t >= 0) & (ids_t < vocab_size)
            if valid.any():
                ids_v = ids_t[valid]
                div_v = div_t[valid]
                logits[i, ids_v] = logits[i, ids_v] * div_v
...
def create_sampler(backend: Optional[str] = None, model_runner=None) -> "Sampler":
    ...
    if backend is None or backend in _BUILT_IN_SAMPLING_BACKENDS:
        return Sampler(model_runner=model_runner)
