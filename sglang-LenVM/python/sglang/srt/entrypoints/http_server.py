...
@app.api_route("/tree_value", methods=["POST", "PUT"])
async def tree_value_request(obj: TreeValueReqInput, request: Request):
    """Handle a tree-attention LVM value request."""
    try:
        ret = await _global_state.tokenizer_manager.generate_request(
            obj, request
        ).__anext__()
...
def _format_values_response(ret):
    """Transform embedding response(s) to a values response.

    Single-request ret: {"embedding": [...], "meta_info": {...}}
    Batch ret: [{"embedding": [...], "meta_info": {...}}, ...]
    Returns: {"values": [[...], ...], "meta_info": meta_info}
    """
    if isinstance(ret, list):
        values = [r["embedding"] for r in ret]
        meta_info = [r["meta_info"] for r in ret]
    else:
        values = [ret["embedding"]]
        meta_info = ret["meta_info"]
    return {"values": values, "meta_info": meta_info}


@app.api_route("/token_value", methods=["POST", "PUT"])
async def token_value_request(obj: EmbeddingReqInput, request: Request):
    """Return per-token LVM values for the given token sequences.

    Request body: {"input_ids": [[t0, t1, ...], ...]}
    Response: {"values": [[v0, v1, ...], ...], "meta_info": ...}
    """
    try:
        ret = await _global_state.tokenizer_manager.generate_request(
            obj, request
        ).__anext__()
        return _format_values_response(ret)
    except ValueError as e:
        return _create_error_response(e)


@app.api_route("/candidate_value", methods=["POST", "PUT"])
async def candidate_value_request(obj: TreeValueReqInput, request: Request):
    """Return per-candidate LVM values using tree-attention (hot decode path).

    Request body: {"input_ids": [[...], ...], "candidate_ids": [[c0, c1, ...], ...]}
    Response: {"values": [[v0, v1, ...], ...], "meta_info": ...}

    Each candidate is evaluated independently against the full prefix.
    EOS token candidates are forced to value 0.0 by the model.
    """
    try:
        ret = await _global_state.tokenizer_manager.generate_request(
            obj, request
