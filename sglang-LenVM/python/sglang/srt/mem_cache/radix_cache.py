...
class RadixCache(BasePrefixCache):
    ...
    def cache_finished_req(self, req: Req, is_insert: bool = True, insert_len: int | None = None):
        ...
        # For special use cases (e.g., tree_value), we only insert a prefix of the request
        # into radix cache to avoid caching tokens with non-standard attention.
        if insert_len is not None:
            insert_len = max(0, min(int(insert_len), len(req.fill_ids), kv_committed_len))
        else:
            insert_len = None
        ...
        self.token_to_kv_pool_allocator.free(kv_indices[len(keys) : kv_committed_len])
