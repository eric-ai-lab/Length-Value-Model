...
class ServerArgs:
    ...
    tokenizer_state_cleanup_delay_s: float = 5.0
    ...
    enable_lvm_guided_sampling: bool = False
    enable_token_temp_scale: bool = False
    lvm_guided_url: Optional[str] = None
    lvm_guided_timeout: float = 5.0
    lvm_guided_bypass_cache: bool = False
    lvm_guided_fn: Optional[str] = None
    # In-proc tree_value (avoid HTTP/JSON). When enabled, the decode server will load a second
    # embedding-style model runner (LVM) in the same scheduler process and call it directly.
    lvm_guided_inproc: bool = False
    lvm_guided_inproc_model_path: Optional[str] = None
    lvm_guided_inproc_model_revision: Optional[str] = None
    lvm_guided_inproc_json_model_override_args: str = "{}"
    lvm_guided_inproc_mem_fraction_static: Optional[float] = None
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--tokenizer-state-cleanup-delay-s",
            type=float,
            default=ServerArgs.tokenizer_state_cleanup_delay_s,
            help="Delay before removing finished request state to tolerate late outputs. "
        ...
        parser.add_argument(
            ...
            action="store_true",
            help="Enable LVM-guided decoding with an external tree_value server.",
        )
        parser.add_argument(
            "--enable-token-temp-scale",
            action="store_true",
            help="Enable per-token temperature scaling via custom_params (no LVM server required).",
        )
        parser.add_argument(
            "--lvm-guided-url",
            type=str,
            default=ServerArgs.lvm_guided_url,
            help="Base URL for LVM server (expects /tree_value).",
        )
        parser.add_argument(
            "--lvm-guided-timeout",
            type=float,
            default=ServerArgs.lvm_guided_timeout,
            help="Timeout in seconds for LVM /tree_value calls.",
        )
        parser.add_argument(
            "--lvm-guided-bypass-cache",
            action="store_true",
            help="Bypass radix cache for LVM /tree_value calls.",
        )
        parser.add_argument(
            "--lvm-guided-fn",
            type=str,
            default=ServerArgs.lvm_guided_fn,
            help="Guidance function spec: import path or 'file.py:func'.",
        )
        parser.add_argument(
            "--lvm-guided-inproc",
            action="store_true",
            help="Run LVM /tree_value in-process (no HTTP/JSON). Requires --lvm-guided-inproc-model-path.",
        )
        parser.add_argument(
            "--lvm-guided-inproc-model-path",
            type=str,
            default=ServerArgs.lvm_guided_inproc_model_path,
            help="Model path for in-proc LVM tree_value runner (embedding-style model).",
        )
        parser.add_argument(
            "--lvm-guided-inproc-model-revision",
            type=nullable_str,
            default=ServerArgs.lvm_guided_inproc_model_revision,
            help="Optional revision for in-proc LVM model.",
        )
        parser.add_argument(
            "--lvm-guided-inproc-json-model-override-args",
            type=str,
            default=ServerArgs.lvm_guided_inproc_json_model_override_args,
            help="JSON string for model override args for in-proc LVM model (e.g. architectures).",
        )
        parser.add_argument(
            "--lvm-guided-inproc-mem-fraction-static",
            type=float,
            default=ServerArgs.lvm_guided_inproc_mem_fraction_static,
            help="mem_fraction_static for the in-proc LVM model runner (defaults to --mem-fraction-static if unset).",
        )
        parser.add_argument(
