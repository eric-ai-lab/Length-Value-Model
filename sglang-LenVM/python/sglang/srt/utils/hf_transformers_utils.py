...
def _sanitize_legacy_extra_special_tokens(
    tokenizer_name: str, kwargs: Dict[str, Any]
) -> None:
    if "extra_special_tokens" in kwargs:
        return

    tokenizer_config_path = Path(tokenizer_name) / "tokenizer_config.json"
    ...
    # Older checkpoints may serialize this field as a list, but newer
    # transformers expects a mapping and crashes before tokenizer.json is used.
    kwargs["extra_special_tokens"] = {}
    logger.warning(
        "Tokenizer config %s stores legacy list-valued extra_special_tokens; "
        "overriding it with an empty mapping for compatibility with the current "
...
def get_tokenizer(
    ...
    **kwargs,
    ...
    _sanitize_legacy_extra_special_tokens(tokenizer_name, kwargs)
