...
from sglang.srt.managers.io_struct import (
    ...
    BatchTokenizedTreeValueReqInput,
    ConfigureLoggingReq,
...
class ReqState:
    ...
    finished: bool
    event: asyncio.Event
    obj: Union[GenerateReqInput, EmbeddingReqInput, TreeValueReqInput]
...
class TokenizerManager(TokenizerCommunicatorMixin, TokenizerManagerMultiItemMixin):
    ...
    async def _tokenize_one_request(
        ...
        if getattr(obj, "input_embeds", None) is not None:
            ...
            input_embeds = obj.input_embeds  # type: ignore[attr-defined]
            input_ids = obj.input_ids  # type: ignore[attr-defined]
        elif getattr(obj, "input_ids", None) is not None:
            input_ids = obj.input_ids  # type: ignore[attr-defined]
        else:
        ...
        if self.mm_processor and hasattr(obj, "contains_mm_input") and obj.contains_mm_input():
        ...
        # TreeValueReqInput uses token ids and custom attention, we still validate vocab range.
    ...
    def _validate_one_request(
        self, obj: Union[GenerateReqInput, EmbeddingReqInput, TreeValueReqInput], input_ids: List[int]
    ) -> None:
        ...
        # Disallow radix cache for TreeValueReqInput unless explicitly requested.
        # Tree attention mask is non-standard causal, so caching those tokens can pollute the radix tree.
        # TreeValueReqInput can still use prefix cache hit; we will prevent inserting the
        # non-causal candidate part into radix cache at scheduler output time.
            else:
                raise ValueError(
                    f"The input ({input_token_num} tokens) is longer than the "
                    f"model's context length ({self.context_len} tokens)."
    ...
    def _create_tokenized_object(
    ...
    ) -> Union[TokenizedGenerateReqInput, TokenizedEmbeddingReqInput, TokenizedTreeValueReqInput]:
        ...
        elif isinstance(obj, TreeValueReqInput):
            # Already tokenized.
            prefix_ids = obj.input_ids
            cand_ids = obj.candidate_ids

            # For single-request calls, TreeValueReqInput still uses the batched JSON shape:
            #   {"input_ids": [[...]], "candidate_ids": [[...]]}
            # Normalize here so downstream logic always sees per-request list[int].
            if isinstance(prefix_ids, list) and len(prefix_ids) > 0 and isinstance(prefix_ids[0], list):
                prefix_ids = prefix_ids[0]
            if isinstance(cand_ids, list) and len(cand_ids) > 0 and isinstance(cand_ids[0], list):
                cand_ids = cand_ids[0]
            if not isinstance(prefix_ids, list) or not all(
                isinstance(x, int) for x in prefix_ids
            ):
                raise ValueError("TreeValueReqInput.input_ids must be a list[int] per request.")
            if not isinstance(cand_ids, list) or not all(isinstance(x, int) for x in cand_ids):
                raise ValueError("TreeValueReqInput.candidate_ids must be a list[int] per request.")

            tokenized_obj = TokenizedTreeValueReqInput(
                prefix_ids=prefix_ids,
                candidate_ids=cand_ids,
                combined_input_ids=prefix_ids + cand_ids,
                prefix_len=len(prefix_ids),
                candidate_len=len(cand_ids),
                bypass_cache=bool(getattr(obj, "bypass_cache", True)),
                rid=obj.rid,
                priority=obj.priority,
                http_worker_ipc=obj.http_worker_ipc,
            )
    ...
    def _batch_has_text(
        self, batch_size: int, obj: Union[GenerateReqInput, EmbeddingReqInput, TreeValueReqInput]
    ) -> bool:
        """Check if any request in the batch contains text input."""
        for i in range(batch_size):
            # TreeValueReqInput does not have `text`; use getattr for compatibility.
            if getattr(obj[i], "text", None):
                return True
            elif self.is_generation and hasattr(obj[i], "contains_mm_input") and obj[i].contains_mm_input():
                return True
    ...
    def _send_one_request(
        ...
        obj: Union[GenerateReqInput, EmbeddingReqInput, TreeValueReqInput],
        tokenized_obj: Union[
            TokenizedGenerateReqInput, TokenizedEmbeddingReqInput, TokenizedTreeValueReqInput
        ],
    ...
    def _send_batch_request(
        ...
        if isinstance(tokenized_objs[0], TokenizedGenerateReqInput):
            batch_req = BatchTokenizedGenerateReqInput(batch=tokenized_objs)  # type: ignore[arg-type]
        elif isinstance(tokenized_objs[0], TokenizedTreeValueReqInput):
            batch_req = BatchTokenizedTreeValueReqInput(batch=tokenized_objs)  # type: ignore[arg-type]
        else:
            batch_req = BatchTokenizedEmbeddingReqInput(batch=tokenized_objs)  # type: ignore[arg-type]
        ...
        for i, rid in enumerate(recv_obj.rids):
            ...
            if state is None:
                finish_reason = (
                    ...
                    if i < len(recv_obj.finished_reasons)
                    else None
                )
                logger.error(
                    "Received output for %s but the state was deleted in TokenizerManager. "
                    "finish_reason=%s",
                    rid,
                    finish_reason,
                )
            ...
            if state.finished:
                ...
                logger.debug(
                    "Skipping output for finished request %s. finish_reason=%s",
                    rid,
                    finish_reason,
                )
                continue
            ...
            if state.finished:
                ...
                # Delay cleanup to avoid late outputs racing with state deletion.
                async def cleanup(request_id: str):
                    delay_s = self.server_args.tokenizer_state_cleanup_delay_s
                    if delay_s > 0:
                        await asyncio.sleep(delay_s)
                    if request_id in self.rid_to_state:
                        del self.rid_to_state[request_id]

                cleanup_task = asyncio.create_task(cleanup(rid))
                self.asyncio_tasks.add(cleanup_task)
                cleanup_task.add_done_callback(self.asyncio_tasks.discard)
