...
class Req:
    ...
    def init_next_round_input(self, tree_cache: Optional[BasePrefixCache] = None):
        ...
        # Tree-value requests should only match prefix tokens in radix cache (never candidates).
        # This enables cache hit/warmup for the prefix while avoiding candidate pollution.
        if bool(getattr(self, "is_tree_value", False)):
            tv_prefix_len = int(getattr(self, "tree_value_prefix_len", 0))
            if tv_prefix_len > 0:
                # Reserve the last prefix token to be computed together with candidates.
                # This avoids needing a full-prefix cache hit (which is blocked by the default
                # input_len-1 rule) and keeps tree_value Q/K lengths consistent.
                max_prefix_len = min(max_prefix_len, max(tv_prefix_len - 1, 0))
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def prepare_for_extend(self):
        ...
        # Init tensors
        reqs = self.reqs
        # Tree-value requests use a custom tree attention mask/positions.
        # Build per-batch spec_info here so ForwardBatch can override positions and
        # attention backends can consume custom_mask.
        if reqs and bool(getattr(reqs[0], "is_tree_value", False)):
            prefix_lens_tv = [int(getattr(r, "tree_value_prefix_len")) for r in reqs]
            cand_lens_tv = [int(getattr(r, "tree_value_candidate_len")) for r in reqs]
            cached_prefix_lens_tv = [len(r.prefix_indices) for r in reqs]
            ...
            custom_mask, positions = build_tree_value_custom_mask_and_positions(
                prefix_lens=prefix_lens_tv,
                candidate_lens=cand_lens_tv,
                cached_prefix_lens=cached_prefix_lens_tv,
                device=torch.device(self.device),
            )
            self.spec_info = TreeValueSpecInput(
                custom_mask=custom_mask,
                positions=positions,
                tree_value_prefix_lens=prefix_lens_tv,
                tree_value_candidate_lens=cand_lens_tv,
                tree_value_cached_prefix_lens=cached_prefix_lens_tv,
            )
