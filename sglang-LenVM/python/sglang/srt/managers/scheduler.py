...
class Scheduler(
    ...
    def init_request_dispatcher(self):
        self._request_dispatcher = TypeBasedDispatcher(
            [
                ...
                (TokenizedTreeValueReqInput, self.handle_tree_value_request),
                (BatchTokenizedGenerateReqInput, self.handle_batch_generate_request),
                (BatchTokenizedEmbeddingReqInput, self.handle_batch_embedding_request),
                (BatchTokenizedTreeValueReqInput, self.handle_batch_tree_value_request),
    ...
    def handle_tree_value_request(self, recv_req: TokenizedTreeValueReqInput):
        """Handle a tree-attention LenVM value request as an embedding-style request."""
        prefix_len = int(recv_req.prefix_len)
        candidate_len = int(recv_req.candidate_len)
        total_len = len(recv_req.combined_input_ids)

        if prefix_len <= 0:
            raise ValueError(f"tree_value prefix_len must be > 0, got {prefix_len}")
        if candidate_len <= 0:
            raise ValueError(f"tree_value candidate_len must be > 0, got {candidate_len}")
        if prefix_len + candidate_len != total_len:
            raise ValueError(
                "tree_value combined_input_ids length mismatch: "
                f"prefix_len({prefix_len}) + candidate_len({candidate_len}) != total_len({total_len})"
            )
        if recv_req.prefix_ids and len(recv_req.prefix_ids) != prefix_len:
            raise ValueError(
                f"tree_value prefix_ids length mismatch: expected {prefix_len}, got {len(recv_req.prefix_ids)}"
            )
        if recv_req.candidate_ids and len(recv_req.candidate_ids) != candidate_len:
            raise ValueError(
                "tree_value candidate_ids length mismatch: "
                f"expected {candidate_len}, got {len(recv_req.candidate_ids)}"
            )
        ...
        req = Req(
            recv_req.rid,
            "",  # origin_input_text
            recv_req.combined_input_ids,
            sampling_params,
            priority=recv_req.priority,
            http_worker_ipc=recv_req.http_worker_ipc,
        )
        req.tokenizer = self.tokenizer
        req.is_tree_value = True  # type: ignore[attr-defined]
        req.tree_value_prefix_len = prefix_len  # type: ignore[attr-defined]
        req.tree_value_candidate_len = candidate_len  # type: ignore[attr-defined]
        req.tree_value_bypass_cache = bool(recv_req.bypass_cache)  # type: ignore[attr-defined]

        tree_cache = None if req.tree_value_bypass_cache else self.tree_cache
    ...
    def handle_batch_tree_value_request(self, recv_req: BatchTokenizedTreeValueReqInput):
        for tokenized_req in recv_req:
            self.handle_tree_value_request(tokenized_req)
    ...
    def _get_new_batch_prefill_raw(
        ...
        # Tree-value requests require a full-tree custom attention mask over the entire
        # (new prefix tokens + all candidates) extend segment. We currently do not support
        # chunking/splitting that mask, so disable chunked prefill for any batch whose
        # head-of-line request is tree_value.
        if self.waiting_queue and bool(getattr(self.waiting_queue[0], "is_tree_value", False)):
        ...
        tree_value_mode = None
        # Hard cap by free request slots. This prevents alloc_req_slots() from crashing
        # when the waiting queue is huge (e.g., very high client concurrency) and only a
        # limited number of req slots are actually available.
        #
        # Note: req_to_token_pool is a *request-slot* pool (not KV token slots). If we try to
        # schedule more requests than available slots, alloc_req_slots will raise.
        max_new_reqs = self.req_to_token_pool.available_size()
        for req in self.waiting_queue:
            is_tree_value = bool(getattr(req, "is_tree_value", False))
            if tree_value_mode is None:
                tree_value_mode = is_tree_value
            elif tree_value_mode != is_tree_value:
                # Do not mix tree_value requests (custom mask/positions) with normal requests.
                continue
            if len(adder.can_run_list) >= max_new_reqs:
                # No request slots left for this scheduling round.
                self.running_batch.batch_is_full = True
                break
