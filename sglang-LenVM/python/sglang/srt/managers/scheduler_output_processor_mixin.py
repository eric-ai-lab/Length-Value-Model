        ...
        if self.is_generation:
            ...
            for i, req in enumerate(batch.reqs):
                ...
                if req.is_chunked <= 0:
                    ...
                    if req.finished():
                        # Tree-value requests may optionally bypass radix cache entirely.
                        # When cache is enabled, only warm the prefix and never insert the
                        # candidate suffix because it was evaluated under a custom tree mask.
                        if bool(getattr(req, "is_tree_value", False)):
                            prefix_len = int(getattr(req, "tree_value_prefix_len", 0))
                            bypass_cache = bool(getattr(req, "tree_value_bypass_cache", False))
                            release_kv_cache(
                                req,
                                self.tree_cache,
                                is_insert=not bypass_cache,
                                insert_len=prefix_len if not bypass_cache else None,
                            )
                        else:
