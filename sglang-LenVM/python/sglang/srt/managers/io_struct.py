...
@dataclass
class TreeValueReqInput(BaseReq, APIServingTimingMixin):
    """Tree-attention value request for Length-Value Model (LVM).

    For each request i:
      - input_ids[i] is the prefix token ids
      - candidate_ids[i] is a list of 1-token candidates (each treated as an independent branch)

    The server will run one forward pass with a custom tree attention mask and return
    per-candidate scalar values as `embedding`.
    """

    # NOTE: BaseReq defines default/kw_only fields. To avoid dataclass init ordering
    # issues ("non-default argument ... follows default argument"), we give these
    # required fields defaults and validate them in normalize_batch_and_arguments().
    input_ids: List[List[int]] = field(default_factory=list)
    candidate_ids: List[List[int]] = field(default_factory=list)

    # Whether to log metrics for this request
    log_metrics: bool = True

    # Dummy sampling params for compatibility with shared validation/tokenization paths.
    # We always force max_new_tokens=0 for tree_value.
    sampling_params: Optional[Union[List[Dict], Dict]] = None

    # Disable radix cache usage/insertion (tree attention mask is non-causal).
    bypass_cache: Optional[Union[List[bool], bool]] = False
    ...
    def normalize_batch_and_arguments(self):
        if self.input_ids is None or self.candidate_ids is None:
            raise ValueError("input_ids and candidate_ids must be provided")
        if len(self.input_ids) != len(self.candidate_ids):
            raise ValueError("input_ids and candidate_ids must have the same batch size")
        if len(self.input_ids) == 0:
            raise ValueError("input_ids must be non-empty")

        self.batch_size = len(self.input_ids)
        self.is_single = self.batch_size == 1

        if self.is_single:
            if self.rid is None:
                self.rid = uuid.uuid4().hex
            if self.bypass_cache is None:
                self.bypass_cache = True
            if self.sampling_params is None:
                self.sampling_params = {}
            assert isinstance(self.sampling_params, dict)
            self.sampling_params["max_new_tokens"] = 0
        else:
            if self.rid is None:
                self.rid = [uuid.uuid4().hex for _ in range(self.batch_size)]
                ...
                assert isinstance(self.rid, list), "The rid should be a list."
            if self.bypass_cache is None:
                self.bypass_cache = [True] * self.batch_size
            elif isinstance(self.bypass_cache, bool):
                self.bypass_cache = [self.bypass_cache] * self.batch_size
            else:
                assert isinstance(self.bypass_cache, list)
                if len(self.bypass_cache) != self.batch_size:
                    raise ValueError("bypass_cache must have the same batch size as input_ids")
            if self.sampling_params is None:
                self.sampling_params = [{}] * self.batch_size
            elif isinstance(self.sampling_params, dict):
                self.sampling_params = [self.sampling_params] * self.batch_size
                ...
                if len(self.sampling_params) != self.batch_size:
                    raise ValueError(
                        "sampling_params must have the same batch size as input_ids"
                    )
            for i in range(self.batch_size):
                self.sampling_params[i]["max_new_tokens"] = 0
    ...
    def __getitem__(self, i):
        ...
        return TreeValueReqInput(
            input_ids=self.input_ids[i],
            candidate_ids=self.candidate_ids[i],
            sampling_params=self.sampling_params[i]
            if isinstance(self.sampling_params, list)
            else self.sampling_params,
            rid=self.rid[i],
            bypass_cache=self.bypass_cache[i] if isinstance(self.bypass_cache, list) else self.bypass_cache,
            priority=self.priority,
            routing_key=self.routing_key,
            http_worker_ipc=self.http_worker_ipc,
            **{field: getattr(self, field) for field in _API_SERVING_TIMING_MIXIN_FIELDS},
        )


@dataclass
class TokenizedTreeValueReqInput(BaseReq):
    prefix_ids: List[int] = field(default_factory=list)
    candidate_ids: List[int] = field(default_factory=list)
    combined_input_ids: List[int] = field(default_factory=list)
    prefix_len: int = 0
    candidate_len: int = 0
...
@dataclass
class BatchTokenizedTreeValueReqInput(BaseBatchReq):
    batch: List[TokenizedTreeValueReqInput] = field(default_factory=list)
